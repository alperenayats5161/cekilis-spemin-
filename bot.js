...
var oyun = [
`🎄 eglenmek icin | s.eglence`,
`🎄 Yeni Özellikler İçin | s.yardım1`,
`🎄 Moderasyon İcin | s.moderasyon`,
`🎄 İstatistik icin | s.İstatistik`,
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea'|| msg.content.toLowerCase() == 'selam') {
          try {
                  ...
                  return msg.reply(
                    'Aleyküm Selam, Hoşgeldin <a:pika:791671771891105824>')
          } catch(err) {
            console.log(err);
          }
      }
...
const voiceChannel = "795223562062790657"
...
client.on("guildMemberRemove", async member => {
  ...
  if (db.has(`sKanal_${member.guild.id}`) == false) return;
    ...
    member.guild.channels.cache.get(channel).send(`**<a:leave:794907983184265217> ${member.user.tag} **Sunucudan Çıktı <a:yonok:775093219712040981>  ! \`${db.fetch(`sayac_${member.guild.id}`)}\` kisiye son \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.memberCount}\` kisi kaldı!`);
});
client.on("guildMemberAdd", async member => {
  ...
  if (db.has(`sKanal_${member.guild.id}`) == false) return;
    ...
    member.guild.channels.cache.get(channel).send(`<a:join:794906326009643020> **${member.user.tag} **Sunucuya Geldi <a:yonok:775093219712040981> ! \`${db.fetch(`sayac_${member.guild.id}`)}\` kisiye son \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.memberCount}\` kisi kaldı!`);
});
